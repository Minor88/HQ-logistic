...
const RequestStatusManager: React.FC = () => {
  ...
  const loadStatuses = async () => {
    try {
      setIsLoading(true);
      const { data } = await api.get<{ results: RequestStatus[] }>('/request-statuses/');
  ...
  const handleAddStatus = async (data: FormData) => {
    try {
      ...
      await api.post('/request-statuses/', payload);
  ...
  const handleUpdateStatus = async (data: FormData) => {
    ...
    try {
      ...
      await api.put(`/request-statuses/${selectedStatus.id}/`, payload);
  ...
  const handleDeleteStatus = async (id: number) => {
    ...
    try {
      await api.delete(`/request-statuses/${id}/`);
  ...
  const handleDrop = async (e: React.DragEvent, { sourceIndex, destinationIndex }: { sourceIndex: number, destinationIndex: number }) => {
    ...
    try {
      ...
      await api.post('/request-statuses/reorder/', {
  ...
  const handleEditClick = (status: RequestStatus) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Статусы заявок</h2>
        <Button onClick={() => {
          form.reset();
      ...
      {isLoading ? (
        ...
        <div className="text-center py-4">Статусы заявок не найдены</div>
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Добавить статус заявки</DialogTitle>
          </DialogHeader>
          <Form {...form}>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Редактировать статус заявки</DialogTitle>
          </DialogHeader>
          <Form {...form}>
